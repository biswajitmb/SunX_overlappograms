...
PRO AIA_SPARSE_EM_INIT_MOD, restore=restore, evenorm=evenorm, save=save, timedepend_date=timedepend_date, $
                        use_lgtaxis = use_lgtaxis, noblend=noblend, xrt=xrt, photospheric=photospheric, bases_sigmas=bases_sigmas, $
                        bases_powers=bases_powers, emunit=emunit, normalize=normalize, chianti94fix=chianti94fix, use_tresp=use_tresp,plate_scale = plate_scale, crosscal_factor=crosscal_factor, chn_all=chn_all,tresp_all=tresp_all

   ; Normalize keyword makes the basis functions normalized by sum. Not recommended
   COMMON AIAEM, Dict, Dictfac, halfts, indices, lgtaxis, dlgt, chiantifix, aia_resp, aia_temp_resp, nchannels, basis_funcs, tresp
   nchannels = 6

   chn_all = ['094','131','171','193','211','335']
   ...
   ENDELSE
      ...
      tresp_all = fltarr(nchannels,ntemp)
      ...
      IF KEYWORD_SET(plate_scale) THEN tresp = tresp*(plate_scale/0.6)^2;correct for new plate-scale
      IF KEYWORD_SET(plate_scale) THEN print,'======= BM_modified: using new plate scale ========='
      for m=0,(nsigmas+npowers)*ntemp-1 do begin
         ...
         Dict[0,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[0,*])
         Dict[1,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[1,*])
         Dict[2,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[2,*])
         Dict[3,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[3,*])
         Dict[4,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[4,*])
         Dict[5,m] =  total(basis_funcs[m,0:ntemp-1]*tresp[5,*])
      endfor
      ...
      IF (N_ELEMENTS(XRT) NE 0) THEN BEGIN
         ...
         FOR d=0,n_elements(XRT)-1 DO BEGIN
          ...
          ; Interpolate response function onto lgtaxis 
            ...
            ntemp = n_elements(lgtaxis)
            for m=0,(nsigmas+npowers)*ntemp-1 do begin
               xrt_resp_bm = 10.^(interpol(alog10(xtemp_resp), alog10(xtemp_coord), lgtaxis))
               IF KEYWORD_SET(plate_scale) THEN xrt_resp_bm = xrt_resp_bm*(plate_scale/1.0)^2
               IF KEYWORD_SET(crosscal_factor) THEN xrt_resp_bm = xrt_resp_bm*crosscal_factor
               Dict[6+d,m] = total(basis_funcs[m,0:ntemp-1]*(xrt_resp_bm))
               tresp_all[6+d,*] = xrt_resp_bm
            endfor
         ENDFOR
      ENDIF
