...
def on_motion_cursor(event,fig, wvl,pix_num,line_label,line_wvl,resp_unit,OutDir=None,namestr='',):
    ...
    IMG_frame0 = np.array(IMG_frame0)
    axes = fig.get_axes()
    ax3 = axes[2]
    ax4 = axes[3]
...
def finalize_line(img_data,EM_map,nFA,mag_resp,cmap, ref_map,fig,delta_pix_no,vig_func_data,vig_nyFA,mid_vig_fa_ind,mid_vig_fa_ind_y,detector_noise, ph2el,resp_unit=0,exposure=0,magixs_sigma=None,IncludePoisonNoise=False,StoreRawDetector=False,RawDetectorDim = [1040,2152],NonActivePix = [50, 8,4],OutDir=None,hdul_all=[],NoFrames=1,pix2wvl=None):
    ...
    if start_point is not None and end_point is not None:
        ...
        for i_ in range(len(EM_map[0,0,:])):
            ...
            if vig_func_data != 1: #when vigneting data is available
        ...
        #display_rotated_data(IMG_Magixs2_raw[:,:,0],fig, ax3,cmap=cmap)
        if OutDir is not None:
            if FOV == 0:
                ...
                hdul_all[0][0].header.append(('exposere',exposure,'second'))
                hdul_all[0][0].header.append(('resp_unit',resp_unit))

            hdul_all[0].append(fits.ImageHDU(IMG_allframe.transpose(2, 0, 1),name='FOV-'+format('%d'%FOV))) ##Data is located from 3rd extension
            hdul_all[0][int(FOV)+2].header.append(('Roll', angle, 'degrees'))
            hdul_all[0][int(FOV)+2].header.append(('Tx', center_point_arcsec.Tx.value,'arcsec'))
            hdul_all[0][int(FOV)+2].header.append(('Ty', center_point_arcsec.Ty.value,'arcsec'))
            hdul_all[0][int(FOV)+2].header.append(('exposere',exposure,'second'))
            hdul_all[0][int(FOV)+2].header.append(('resp_unit',resp_unit))
            print(start_point)
            hdul_all[0][int(FOV)+2].header.append(('LOS_p1',str(tuple(float(val) for val in start_point)),'pix'))
            hdul_all[0][int(FOV)+2].header.append(('LOS_p2',str(tuple(float(val) for val in end_point)),'pix'))            
 
            hdul_all[1].append(fits.ImageHDU(Orig_IMG,name='FOV-'+format('%d'%FOV)))
            hdul_all[1][int(FOV)].header.append(('Roll', angle, 'degrees'))
            hdul_all[1][int(FOV)].header.append(('Tx', center_point_arcsec.Tx.value,'arcsec'))
            hdul_all[1][int(FOV)].header.append(('Ty', center_point_arcsec.Ty.value,'arcsec'))

            hdul_all[3].append(fits.ImageHDU(EM_new.transpose(2, 0, 1),name='FOV-'+format('%d'%FOV)))
            hdul_all[3][int(FOV)+1].header.append(('Roll', angle, 'degrees'))
            hdul_all[3][int(FOV)+1].header.append(('Tx', center_point_arcsec.Tx.value,'arcsec'))
            hdul_all[3][int(FOV)+1].header.append(('Ty', center_point_arcsec.Ty.value,'arcsec'))
            ...
            hdul_all[2].write('%0.3f\t'%end_point[0]) 
            hdul_all[2].write('%0.3f\t'%end_point[1])
            hdul_all[2].write('%0.3f\t'%start_point[0])
            hdul_all[2].write('%0.3f\n'%start_point[1])

            if StoreRawDetector is True:
                hdul_all[4].append(fits.ImageHDU(IMG_Magixs2_raw.transpose(2, 0, 1),name='FOV-'+format('%d'%FOV)))
                hdul_all[4][int(FOV)].header.append(('Roll', angle, 'degrees'))
                hdul_all[4][int(FOV)].header.append(('Tx', center_point_arcsec.Tx.value,'arcsec'))
                hdul_all[4][int(FOV)].header.append(('Ty', center_point_arcsec.Ty.value,'arcsec'))
                hdul_all[4][int(FOV)].header.append(('exposere',exposure,'second'))
                hdul_all[4][int(FOV)].header.append(('resp_unit',resp_unit))
                hdul_all[4][int(FOV)].header.append(('LOS_p1',str(tuple(float(val) for val in start_point)),'pix'))
                hdul_all[4][int(FOV)].header.append(('LOS_p2',str(tuple(float(val) for val in end_point)),'pix'))
            '''
...
def copy_fits_ext(original_fits_file, index, OutFile = 'new_file.fits'):
    with fits.open(original_fits_file) as hdul:
        hdu_to_copy = hdul[index]
        new_primary_hdu = fits.PrimaryHDU(data=hdu_to_copy.data, header=hdu_to_copy.header)
        new_hdul = fits.HDUList([new_primary_hdu])
        #new_hdul = fits.HDUList([fits.PrimaryHDU(), hdu_to_copy])
        new_hdul.writeto(OutFile, overwrite=True)


def make_weight_map(overlappogram_file,syst = 0.1,FOV = 1, OutFile = None):
    hdul = fits.open(overlappogram_file)
    overlappogram = hdul[FOV+1]
    if hdul[FOV+1].header['RESP_UNIT'] == 'electron': 
        ph2el = hdul[1].data
    elif hdul[1].header['RESP_UNIT'] == 'photon':
        ph2el = 1
    da = overlappogram.data/ph2el
    error = np.sqrt(da) / float(overlappogram.header['EXPOSERE'])#ph
    sys_error = (da*syst) / float(overlappogram.header['EXPOSERE'])
    error = np.sqrt(error*2 + sys_error**2)*ph2el 
    weight_map = 1/error
    weight_map = np.nan_to_num(weight_map) 
    if OutFile is not None:
        hdu_to_copy = overlappogram
        hdu_to_copy.data = weight_map 
        new_primary_hdu = fits.PrimaryHDU(data=hdu_to_copy.data, header=hdu_to_copy.header)
        new_hdul = fits.HDUList([new_primary_hdu])
        new_hdul.writeto(OutFile, overwrite=True)
