...
class fwd(object):
    ...
    def create_ovlp(self,img_data,EM_map,nFA,mag_resp,wvl,pix_num,cmap, ref_map,delta_pix_no,vig_func_data,vig_nyFA,mid_vig_fa_ind,mid_vig_fa_ind_y,detector_noise, ph2el,resp_unit=0,exposure=0,magixs_sigma = None, IncludePoisonNoise = False, StoreRawDetector = False, RawDetectorDim = [1040,2152],NonActivePix = [50, 8,4],OutDir = None,hdul_all=[],pointing_info_file = None,SavePlot=False,line_label=[],line_wvl=[],namestr='',NoFrames=1,pix2wvl=None):
        ...
        FOV,botx,boty,topx,topy = np.loadtxt(pointing_info_file,unpack=True)
        for k in range(len(botx)): 
            ...
            if OutDir is not None:
                ...
                hdul_all[0].append(fits.ImageHDU(IMG_allframe.transpose(2, 0, 1),name='FOV-'+format('%d'%FOV[k])))
                hdul_all[0][int(FOV[k])-1].header.append(('Roll', angle))
                hdul_all[0][int(FOV[k])-1].header.append(('Tx', center_point_arcsec.Tx.value))
                hdul_all[0][int(FOV[k])-1].header.append(('Ty', center_point_arcsec.Ty.value))
                hdul_all[0][int(FOV[k])-1].header.append(('exposere',exposure))
                hdul_all[0][int(FOV[k])-1].header.append(('resp_unit',resp_unit))
    
                hdul_all[1].append(fits.ImageHDU(Orig_IMG,name='FOV-'+format('%d'%FOV[k])))
                hdul_all[1][int(FOV[k])-1].header.append(('Roll', angle))
                hdul_all[1][int(FOV[k])-1].header.append(('Tx', center_point_arcsec.Tx.value))
                hdul_all[1][int(FOV[k])-1].header.append(('Ty', center_point_arcsec.Ty.value))
    
                hdul_all[3].append(fits.ImageHDU(EM_new.transpose(2, 0, 1),name='FOV-'+format('%d'%FOV[k])))
                hdul_all[3][int(FOV[k])-1].header.append(('Roll', angle))
                hdul_all[3][int(FOV[k])-1].header.append(('Tx', center_point_arcsec.Tx.value))
                hdul_all[3][int(FOV[k])-1].header.append(('Ty', center_point_arcsec.Ty.value))
    
                hdul_all[2].write('%d\t'%(FOV[k]+1))
    ...
    def magixs_fwd(self, FullSun_img_data, resp_file,Wvl_cal_file, resp_unit = None, plate_scale = None,Vigneting_func_file=None,psf=None,exposure = 1, line_label = [],line_wvl = [],cmap='hot',StoreRawDetector = False,RawDetectorDim = [1040,2152],NonActivePix = [50, 8,4], IncludePoisonNoise=False,IncludeDetecorNoise=False,detector_noise = [],phot2elec_conv_file = None,StoreOutputs = False,OutDir=None,namestr='MaGIXS2_',pointing_info_file=None,SavePlot = False,NoFrames=1):
        ...
        if StoreOutputs is True:
            ...
            pointing_info.write('# FOV, Roll (deg), Solar-X (arc-sec), Solar-Y (arc-sec)\n')
    ...
    def run_inversion(self,dataDir,overlappogram_file, resp_file, gnt_file,configfile, weight_file = None, OutDir=None, prefix = "MaGIXS2_inversion",FOV=1):
        ...
        search_and_replace_line(configfile, 'prefix','prefix="'+os.path.join(OutDir,prefix)+'"')
    ...
    def plot_detector(self,ref_map,Overlappogram_file,EMin_file,cmap='hot',line_label=[],line_wvl=[],FOV = 1, FrameRange = [0,1],unit='electron'):
        ...
        with fits.open(Overlappogram_file) as hdul:
            ...
            exposure = hdul[0].header['EXPOSERE']
            roll = hdul[0].header['ROLL']
            Tx = hdul[0].header['Tx']
            Ty = hdul[0].header['Ty']
        center_point = ref_map.world_to_pixel(SkyCoord(Tx*u.arcsec,Ty*u.arcsec,frame=ref_map.coordinate_frame))
        start_point,end_point = get_line_start_end(-roll, center_point.x.value,center_point.y.value,length = 800)
    ...
    def plot_detector_avg(self,ref_map,Overlappogram_file,EMin_file,SpecPure_file=None,cmap='hot',line_label=[],line_wvl=[],FOV = 1,specInd = [7,12,5],Pscale = [2.8*2.8,2.8],unit='electron'):
        ...
        if SpecPure_file is not None:
            ...
            add_colorbar(ax5, dataSpec[specInd[0],:,:], cmap, 'Ph cm$^{-2}$ s$^{-1}$')

            ax6.imshow(dataSpec[specInd[1],:,:],origin='lower',cmap=cmap)
            add_colorbar(ax6, dataSpec[specInd[1],:,:], cmap, 'Ph cm$^{-2}$ s$^{-1}$')

            ax7.imshow(dataSpec[specInd[2],:,:],origin='lower',cmap=cmap)
            add_colorbar(ax7, dataSpec[specInd[2],:,:], cmap, 'Ph cm$^{-2}$ s$^{-1}$')
